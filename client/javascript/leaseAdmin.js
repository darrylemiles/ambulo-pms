        ...
        class LeaseManager {
            constructor() {
                ...
                this.editMode = false;
                this.leaseIdCounter = Math.max(...this.leases.map(l => parseInt(l.id.replace('LSE', ''))), 0);
                this.uploadedFiles = [];
                this.tenantNames = {
                    1: 'John Smith',
                    2: 'Sarah Johnson', 
                ...
                return [
                    {
                        ...
                        notes: 'Reliable tenant with excellent payment history. Property includes parking space.',
                    ...
                    },
                    {
                        ...
                        notes: 'Commercial lease for small business. Needs renewal discussion for extension.',
                        createdAt: new Date().toISOString(),
                        updatedAt: new Date().toISOString()
                    },
                    {
                        ...
                        notes: 'First-time renter, requires occasional follow-up on payment schedules.',
                        createdAt: new Date().toISOString(),
                        updatedAt: new Date().toISOString()
                    }
                ];
            ...
            filterLeases(filters) {
                return this.leases.filter(lease => {
                    ...
                    if (filters.tenant && !lease.tenantName.toLowerCase().includes(filters.tenant.toLowerCase())) return false;
                    if (filters.property && !lease.propertyName.toLowerCase().includes(filters.property.toLowerCase())) return false;
                    if (filters.date) {
        ...
        function loadLeaseTable() {
            ...
            leases.forEach(lease => {
                ...
                row.innerHTML = `
                    <td>
                        <strong style="color: #1f2937; font-weight: 700;">${lease.id}</strong>
                    </td>
                    <td>
                        <div style="font-weight: 600; color: #111827;">${lease.tenantName}</div>
                        <div style="font-size: 12px; color: #6b7280;">ID: ${lease.tenantId}</div>
                    </td>
                    <td>
                        <div style="font-weight: 500; color: #111827;">${lease.propertyName}</div>
                        <div style="font-size: 12px; color: #6b7280;">Property ID: ${lease.propertyId}</div>
                    </td>
                    <td>
                        <div style="font-size: 13px; font-weight: 500;">${formatDate(lease.startDate)}</div>
                        <div style="font-size: 12px; color: #6b7280;">to ${formatDate(lease.endDate)}</div>
                        <div style="font-size: 11px; color: #9ca3af;">${getDuration(lease.startDate, lease.endDate)}</div>
                    </td>
                    <td>
                        <span class="status-badge status-${lease.status.toLowerCase()}">${lease.status}</span>
                    </td>
                    <td>
                        <div style="font-weight: 700; color: #059669; font-size: 16px;">₱${lease.monthlyRent.toLocaleString()}</div>
                        <div style="font-size: 12px; color: #6b7280;">${lease.paymentFrequency}</div>
                    </td>
                    <td>
                        <div style="font-weight: 500;">${getNextDueDate(lease)}</div>
                    </td>
                    <td>
                        <button class="action-btn action-view" onclick="showDetailView('${lease.id}')" title="View Details"><i class="fa-solid fa-eye"></i></button>
                        <button class="action-btn action-edit" onclick="showEditView('${lease.id}')" title="Edit Lease"><i class="fa-solid fa-pen"></i></button>
                        <button class="action-btn action-delete" onclick="showDeleteModal('${lease.id}')" title="Delete Lease"><i class="fa-solid fa-trash"></i></button>
                    </td>
                `;
        ...
        function getNextDueDate(lease) {
            ...
            if (lease.status !== 'ACTIVE') return '<span style="color: #6b7280;">N/A</span>';
        ...
        function loadForm(lease) {
            ...
            document.getElementById('paymentFrequency').value = lease.paymentFrequency || 'Monthly';
            document.getElementById('quarterlyTax').value = lease.quarterlyTax || '';
            document.getElementById('securityDeposit').value = lease.securityDeposit || '';
            ...
            document.getElementById('isSecurityRefundable').checked = lease.isSecurityRefundable !== false;
            document.getElementById('advanceForfeited').checked = lease.advanceForfeited === true;
            document.getElementById('autoTerminationMonths').value = lease.autoTerminationMonths || '';
            document.getElementById('terminationTriggerDays').value = lease.terminationTriggerDays || 61;
            document.getElementById('noticeCancelDays').value = lease.noticeCancelDays || '';
            document.getElementById('noticeRenewalDays').value = lease.noticeRenewalDays || '';
            document.getElementById('rentIncreaseRenewal').value = lease.rentIncreaseRenewal || '';
        ...
        function saveLease() {
            ...
            setTimeout(() => {
                const formData = {
                    ...
                    tenantName: leaseManager.tenantNames[document.getElementById('tenantId').value],
                    propertyId: parseInt(document.getElementById('propertyId').value),
                    propertyName: leaseManager.propertyNames[document.getElementById('propertyId').value],
                    ...
                    quarterlyTax: parseFloat(document.getElementById('quarterlyTax').value) || 0,
                    ...
                    isSecurityRefundable: document.getElementById('isSecurityRefundable').checked,
                    advanceForfeited: document.getElementById('advanceForfeited').checked,
                    autoTerminationMonths: parseInt(document.getElementById('autoTerminationMonths').value) || 0,
                    terminationTriggerDays: parseInt(document.getElementById('terminationTriggerDays').value) || 61,
                    noticeCancelDays: parseInt(document.getElementById('noticeCancelDays').value) || 0,
                    noticeRenewalDays: parseInt(document.getElementById('noticeRenewalDays').value) || 0,
                    rentIncreaseRenewal: parseFloat(document.getElementById('rentIncreaseRenewal').value) || 0,
        ...
        function showCancelModal() {
            ...
            if (leaseManager.editMode) {
                message.textContent = 'Are you sure you want to cancel editing this lease? Any unsaved changes will be lost.';
            } else {
                message.textContent = 'Are you sure you want to cancel creating this lease? Any entered data will be lost.';
            }
        ...
        function checkForUnsavedChanges() {
            ...
            const formFields = [
                'tenantId', 'propertyId', 'startDate', 'endDate', 'status',
                'monthlyRent', 'paymentFrequency', 'quarterlyTax', 'securityDeposit',
                'advancePayment', 'lateFee', 'gracePeriod', 'autoTerminationMonths',
                'terminationTriggerDays', 'noticeCancelDays', 'noticeRenewalDays',
                'rentIncreaseRenewal', 'notes'
            ];
            
            return formFields.some(fieldId => {
                const field = document.getElementById(fieldId);
        ...
        function loadDetailView(lease) {
            ...
            document.getElementById('detailSubtitle').textContent = `${lease.tenantName} • ${lease.propertyName}`;
            ...
            document.getElementById('basicInfo').innerHTML = `
                ...
                <div class="info-item">
                    ...
                    <div class="info-value">${getDuration(lease.startDate, lease.endDate)}</div>
                </div>
            `;
            ...
            document.getElementById('financialInfo').innerHTML = `
                ...
                <div class="info-item">
                    ...
                    <div class="info-value">${lease.securityDeposit} month(s)</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Advance Payment</div>
                    <div class="info-value">${lease.advancePayment} month(s)</div>
                </div>
                <div class="info-item">
            ...
            document.getElementById('rulesInfo').innerHTML = `
                <div class="info-item">
                    ...
                    <div class="info-value">${lease.isSecurityRefundable ? 'Yes' : 'No'}</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Advance Forfeited</div>
                    <div class="info-value">${lease.advanceForfeited ? 'Yes' : 'No'}</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Auto-Termination</div>
                    <div class="info-value">${lease.autoTerminationMonths} month(s)</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Termination Trigger</div>
                    <div class="info-value">${lease.terminationTriggerDays} days</div>
                </div>
                <div class="info-item">
                ...
                <div class="info-item">
                    ...
                    <div class="info-value">${lease.noticeRenewalDays} days</div>
                </div>
                <div class="info-item">
            ...
            document.getElementById('amountDue').textContent = `₱${lease.monthlyRent.toLocaleString()}`;
        ...
        function updateUploadedFilesList() {
            ...
            container.innerHTML = leaseManager.uploadedFiles.map((file, index) => `
                <div class="file-item">
                    <div class="file-info">
