...
function openModal(id) {
    ...
    try {
        var btype = currentSubmission.business_type || currentSubmission.businessType || currentSubmission.company_type || currentSubmission.type || '';
    ...
    try {
        var prefSize = currentSubmission.preferred_space_size || currentSubmission.preferredSpaceSize || currentSubmission.space_size || '';
        var budgetRaw = currentSubmission.monthly_budget_range || currentSubmission.monthly_budget || currentSubmission.budget_range || currentSubmission.budget || '';
        ...
        if (prefSize) {
            ...
            if (mRange) {
                ...
                if (!isNaN(n1) && !isNaN(n2)) {
                    var formattedRange = n1.toLocaleString() + '–' + n2.toLocaleString() + ' sqm';
                    ...
                    parsedPrimary = n1;
                    parsedSecondary = n2;
                    displaySize = (category ? category + ' (' + formattedRange + ')' : formattedRange);
                }
            } else {
            ...
            if (!displaySize && category) {
                ...
                if (canonical) displaySize = category + (canonical ? ' (' + canonical + ')' : '');
            ...
            if (sizeEl) {
                        ...
                        if (rawNumMatch) {
                            ...
                            if (!isNaN(rawNum)) {
                                ...
                                if (parsedSecondary !== null) {
                                    if (rawNum < parsedPrimary - 0.01 || rawNum > parsedSecondary + 0.01) showRaw = true;
                                } else {
                ...
                } catch (e) { showRaw = true; }

                if (displaySize) {
                    if (showRaw) {
                        sizeEl.innerHTML = escapeHtml(displaySize) + ' <small style="color:var(--text-muted); font-size:0.9rem;">' + escapeHtml(rawSize) + '</small>';
                    } else {
        ...
        if (budgetRaw) {
            ...
            if (mRange) {
                var n1 = Number(mRange[1].replace(/[^0-9.]/g, ''));
                var n2 = Number(mRange[2].replace(/[^0-9.]/g, ''));
                if (!isNaN(n1) && !isNaN(n2)) budgetText = '₱' + n1.toLocaleString() + ' – ₱' + n2.toLocaleString();
            } else if (mUnder) {
